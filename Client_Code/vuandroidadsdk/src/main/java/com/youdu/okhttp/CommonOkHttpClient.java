...
public class CommonOkHttpClient {
    ...
    //静态语句块的方式配置参数，所有的参数一致如超时参数
    static {
        OkHttpClient.Builder okHttpClientBuilder = new OkHttpClient.Builder();
        okHttpClientBuilder.hostnameVerifier(new HostnameVerifier() {   //https支持
            @Override
            public boolean verify(String hostname, SSLSession session) {
                return true;    //无论什么主机都return true
            }
        });
        ...
        okHttpClientBuilder.connectTimeout(TIME_OUT, TimeUnit.SECONDS); //超时时间
        okHttpClientBuilder.readTimeout(TIME_OUT, TimeUnit.SECONDS);
        okHttpClientBuilder.writeTimeout(TIME_OUT, TimeUnit.SECONDS);
        okHttpClientBuilder.followRedirects(true);  //运行请求重定向
        /**
         * trust all the https point ,https支持
         */
